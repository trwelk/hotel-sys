...
function MenuEditable(props) {
  ...
  const exportCsv = () =>{
    let Row=[];
    let A = [['Menu Id','Menu Name']];
    let re = Menu;
    console.log(Menu);

    for(let item = 0;item < re.length;item++){
      A.push([re[item].id,re[item].menuName])
    }
    console.warn(A);
  ...
  const table = data ? (
    <div>
      <ButtonGroup>
      <Button variant="contained" color="primary" colohref='/fnb/production/newMenu'>
        Add a Menu
      </Button>
